...
const References = () => {
  ...
  return (
    <main className="p-4 sm:p-8 md:p-10 lg:p-14 xl:p-20">
      {
        loading ? 'Loading...' :
        dbReferences.length === 0 && searchQuery === '' ? 'Your references will be displayed here...' : 
        references.length === 0 ? 'No Results Found' : 
        references.map( reference => <Reference key = { reference.id } reference = { reference } /> )
      }
    </main>
  )
}
