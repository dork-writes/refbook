  ...
  return (
    <header className="sticky top-0 shadow-md z-10 space-y-5 md:space-y-0 md:space-x-2 md:flex md:justify-between py-7 px-5 md:p-10 bg-zinc-900 w-[100%] opacity-90">
        ...
        <div className="flex md:w-[40%] bg-zinc-800 px-2 rounded-md">
          ...
          <input autoComplete = "off" onChange = { filterChange } value = { searchQuery } tabIndex={ -1 } className="px-2 py-2 rounded-md w-[100%] bg-transparent focus:outline-0" placeholder = "Search" type = "text"/>
        </div>
        <button onClick = { setForm } tabIndex={-1} className="block px-5 py-2 bg-slate-700 hover:bg-slate-600 active:bg-slate-700 w-[100%] md:w-fit transition-all rounded-sm">Add a reference</button>
    </header> 
  )
}
